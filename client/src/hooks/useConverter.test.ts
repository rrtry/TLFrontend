...
const defaultPriceChangesState = {
  data: [
...
describe('useConverter', () => {
  beforeEach(() => {
    ...
    vi.mocked(usePriceChanges).mockReturnValue(defaultPriceChangesState);
  });
  ...
  it('рассчитывает rate как последнюю цену из истории', async () => {
    const { result } = renderHook(() => useConverter());
  ...
  it('передаёт правильные параметры в usePriceChanges', async () => {
    renderHook(() => useConverter());
    
    await act(async () => {
      await vi.runAllTimersAsync();
    });

    expect(usePriceChanges).toHaveBeenCalledWith('CAD', 'PLN', 1);
  });

  it('обновляет usePriceChanges при смене периода', async () => {
    const { result } = renderHook(() => useConverter());
    await act(async () => {
      await vi.runAllTimersAsync();
    });
    ...
    act(() => {
      result.current.setPeriod(5);
    });

    expect(usePriceChanges).toHaveBeenLastCalledWith('CAD', 'PLN', 5);
  });

  it('пробрасывает rateLoading и rateError из usePriceChanges', async () => {
    vi.mocked(usePriceChanges).mockReturnValue({
      ...defaultPriceChangesState,
      loading: true,
      error: 'Error',
    });
    ...
    expect(result.current.rateLoading).toBe(true);
    expect(result.current.rateError).toBe('Error');
  });

  it('корректно обрабатывает пустую историю (rate = 0)', async () => {
    vi.mocked(usePriceChanges).mockReturnValue({
      data: [],
      loading: false,
      error: null,
